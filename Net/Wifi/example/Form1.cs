...
namespace MyUtil.Net
{
    public partial class Form1 : Form
    {
        ...
        void WlanIface_WlanConnectionNotification(Wlan.WlanNotificationData notifyData, Wlan.WlanConnectionNotificationData connNotifyData)
        {
            ...
            {
                //连接失败
                if ((Wlan.WlanNotificationCodeAcm)notifyData.NotificationCode == Wlan.WlanNotificationCodeAcm.ConnectionAttemptFail)
                {
                    Invoke(new Action(() =>
                    {
                        label2.Text = "未连接";
                ...
                if ((Wlan.WlanNotificationCodeAcm)notifyData.NotificationCode == Wlan.WlanNotificationCodeAcm.Disconnected)
                {
                    Invoke(new Action(() =>
                    {
                        label2.Text = "未连接";
                ...
                if ((Wlan.WlanNotificationCodeAcm)notifyData.NotificationCode == Wlan.WlanNotificationCodeAcm.Disconnecting)
                {
                    Invoke(new Action(() =>
                    {
                        label2.Text = "未连接";
                ...
                if ((Wlan.WlanNotificationCodeAcm)notifyData.NotificationCode == Wlan.WlanNotificationCodeAcm.ConnectionStart)
                {
                    Invoke(new Action(() =>
                    {
                        label2.Text = "连接中…";
